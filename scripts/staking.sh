...
# check dusd balance 
dfx canister call dusd_ledger icrc1_balance_of '(record {owner= principal "5g24m-kxyrd-yb7wl-up5k6-4egww-miul7-gajat-e2d7i-mdpc7-6dduf-eae"})' --identity default

# transfer ckusdc to stable coin minter address to get same amount of dusd
...
dfx canister call stablecoin_minter notify_mint_with_ckusdc '(record{ ckusdc_block_index=3; minting_token=variant {DUSD}})' --identity default


# you can check again dusd balance 
dfx canister call dusd_ledger icrc1_balance_of '(record {owner= principal "5g24m-kxyrd-yb7wl-up5k6-4egww-miul7-gajat-e2d7i-mdpc7-6dduf-eae"})' --identity default

# transfer dusd to staking canister 
dfx canister call dusd_ledger icrc1_transfer '(record{ to=record {owner = principal "mhahe-xqaaa-aaaag-qndha-cai"; subaccount= null}; amount = 10_000_000;})' --identity default
