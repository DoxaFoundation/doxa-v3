...
chmod +x ./scripts/deploy-template-ledger-local.sh
...
CANISTER='ckusdc_ledger'

./scripts/deploy-template-ledger-local.sh $Decimals "$TOKEN_SYMBOL" "$TOKEN_NAME" "$TRANSFER_FEE" "$METADATA" "$CANISTER" 1000
...
./scripts/deploy-template-ledger-local.sh $Decimals "$TOKEN_SYMBOL" "$TOKEN_NAME" "$TRANSFER_FEE" "$METADATA" "$CANISTER" 2
...
./scripts/deploy-template-ledger-local.sh $Decimals "$TOKEN_SYMBOL" "$TOKEN_NAME" "$TRANSFER_FEE" "$METADATA" "$CANISTER" 5
...
./scripts/deploy-template-ledger-local.sh $Decimals "$TOKEN_SYMBOL" "$TOKEN_NAME" "$TRANSFER_FEE" "$METADATA" "$CANISTER" 8000
