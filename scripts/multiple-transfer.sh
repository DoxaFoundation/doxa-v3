...
echo "Transfer: $i"
if [ $((i % 2)) -eq 0 ]; then
 dfx canister call usdx_ledger icrc1_transfer "(record {to=record{owner= principal \"$Doxa\"}; amount=1_000_000})"  --identity default
else
 dfx canister call usdx_ledger icrc1_transfer "(record {to=record{owner= principal \"$Default\"}; amount=1_000_000})" --identity doxa
fi
done
